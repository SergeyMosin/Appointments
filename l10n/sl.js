OC.L10N.register(
    "appointments",
    {
    "%s Appointment is Confirmed" : "%s: sestanek je potrjen",
    "Your %1$s appointment scheduled for %2$s is now confirmed." : " %1$s: sestanek %2$s je potrjen.",
    "Your %s appointment is confirmed." : "Sestanek %s je potrjen.",
    "%s Appointment is Canceled" : "%s: sestanek je preklican",
    "Your %1$s appointment scheduled for %2$s is now canceled." : " %1$s: sestanek %2$s je preklican.",
    "Your %s appointment is canceled." : "Sestanek %s je preklican.",
    "Appointment is Canceled" : "Sestanek je preklican",
    "%s Appointment" : "%s: sestanek",
    "Thank you" : "Hvala",
    "%s Appointment (action needed)" : "%s: sestanek (zahtevan je odziv)",
    "Dear %s," : "Spoštovani %s,",
    "The %1$s appointment scheduled for %2$s is awaiting your confirmation." : "Sestanek, ki ga pripravlja %1$s in je predviden na %2$s, čaka na vašo potrditev.",
    "Confirm" : "Potrdi",
    "Cancel" : "Prekliči",
    "Info" : "Podrobnosti",
    "Link Expired..." : "Povezava je pretekla ...",
    "Appointments" : "Sestanki",
    "Public Page [Online]" : "Javna stran [povezano]",
    "Public Page [Disabled]" : "Javna stran [onemogočeno]",
    "Add appointments" : "Dodaj sestanke",
    "Customize Public Page" : "Javna stran po meri",
    "Organization Name" : "Ime ustanove",
    "Organization Address" : "Naslov ustanove",
    "Email" : "Elektronski naslov",
    "Phone (optional)" : "Telefonska številka (izbirno)",
    "Attach .ics file to confirmation email" : "Priloži datoteko koledarja .ics k potrditvenemu sporočilu",
    "Show Notifications on Status Change" : "Pokaži obvestila ob spremembi stanja udeležbe",
    "Use Google reCAPTCHA" : "Uporabi Google reCAPTCHA",
    "Add to Calendar" : "Dodaj v koledar",
    "Discard" : "Opusti",
    "Remove All" : "Odstrani vse",
    "Copy to Next" : "Kopiraj v Next",
    "Close" : "Zapri",
    "Public Page Preview" : "Predogled javne strani",
    "Copy public link" : "Kopiraj javno povezavo",
    "Schedule Generator" : "Ustvarjalnik urnika",
    "Add open appointments to you calendar" : "Dodaj odprte sestanke v koledar",
    "New Settings Applied." : "Nove nastavitve so uveljavljene",
    "Address" : "Naslov",
    "One Week" : "En teden",
    "Two Weeks" : "Dva tedna",
    "Three Weeks" : "Tri tedne ",
    "Four Weeks" : "Štiri tedne",
    "Five Weeks" : "Pet tednov",
    "GDPR Compliance" : "Skladnost z GDPR",
    "Apply" : "Uveljavi",
    "Select Dates:" : "Izbor datumov:",
    "Select Dates" : "Izbor datumov",
    "Appointment Duration:" : "Trajanje sestanka:",
    "Timezone:" : "Časovni pas:",
    "Start" : "Začni",
    "close" : "zapri",
    "Date & Time:" : "Datum in ura:",
    "Select Date & Time" : "Izbor datuma in ure",
    "Name:" : "Ime:",
    "Enter Name" : "Vpis imena",
    "Email:" : "Elektronski naslov:",
    "Enter Email" : "Vpis elektronskega naslova",
    "Phone:" : "Telefon:",
    "Enter Phone Number" : "Vpis telefonske številke",
    "An Error Has Occurred" : "Prišlo je do napake",
    "Please try again later" : "Poskusite znova kasneje"
},
"nplurals=4; plural=(n%100==1 ? 0 : n%100==2 ? 1 : n%100==3 || n%100==4 ? 2 : 3);");
